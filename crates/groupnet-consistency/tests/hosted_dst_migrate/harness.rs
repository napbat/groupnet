//! `hosted_dst_migrate.rs`'s own cluster harness: the engines, the lagging
//! watch, the follower loop, the recovery gate, the migration-heavy fault
//! schedule, and the scenario drawn from it. The model it drives — the feed
//! ring's layout, the ledger fold, the tally — and the properties it is
//! measured against stay next door in `hosted_dst_migrate.rs`.
...
use super::Count::{
    Acked, FenceBlocked, FenceSeen, GapRecovered, Ghost, Migration, Recovered, Restart, RingGap,
    S5Obligation,
};
use super::{
    Advance, Entries, InFlight, LAG, Lead, MAX_INFLIGHT, Node, RING, ROUNDS, Ring, SLOTS, Stats,
    Tag, ZERO, cfg, cursor, feed_entry_key, ring, rng, set, tok, write_key,
};
...
struct Harness {
    ...
    /// rules are denominated over; and each node's watch lag.
    all: Vec<NodeId>,
    voters: BTreeSet<NodeId>,
    majority: usize,
    lags: BTreeMap<NodeId, usize>,
...
impl Harness {
    /// A cluster of `n` nodes over a roster of three, each with a watch lagging
    /// `0..lag` steps. The roster is rotated, so it is not always the low ids
    /// and the rendezvous top of a run is sometimes outside it.
    fn new(tag: Tag, n: u32, rng: &mut SplitMix64, (cap, lag): (u64, u32)) -> Self {
        ...
        let (mut nodes, mut lags) = (BTreeMap::new(), BTreeMap::new());
        for id in &all {
            ...
            sim.add(engine);
            let lag = usize::try_from(rng.below(lag.max(1))).expect("a 0..4 draw");
            lags.insert(id.clone(), lag);
            nodes.insert(id.clone(), Node::new(id, lag, Watermarks::new()));
        }
        Self {
            tag,
            group,
    ...
    /// Pushes this round's engine reading into `id`'s watch, and returns what
    /// the watch now reports: the reading from `lag` steps ago.
    fn advance_watch(&mut self, id: &NodeId) -> Lead {
        let engine = self.sim.leadership_of(id).unwrap_or((0, None));
        let node = self.nodes.get_mut(id).expect("a live node");
        node.watch.push_back(engine);
        while node.watch.len() > node.lag + 1 {
            node.watch.pop_front();
        }
        node.lead()
    }
    ...
    ///
    /// Negative control, re-derived against this corpus: bypassing the recovery
    /// gate here does **not** fire — all 64 seeds still pass. That is honest and
    /// worth recording: a hand-over alone does not strand a successor far enough
    /// behind, and what makes S5 bite under a bypassed gate is the **stall arm**
    /// of `hosted_dst.rs`'s wider schedule (which fires there at seed 27). This
    /// file's S5 is a check on every serving observation, not a trap for that
    /// particular bug.
    ...
    fn poll_writes(&mut self) {
        ...
        for id in self.live_ids() {
            ...
            for mut wait in waits {
                let verdict = CommitCore::evaluate(&view, &id, wait.token, Commit::QuorumApplied);
                let committed = verdict.is_committed();
                self.check_fence(&id, &mut wait, (&view, committed));
                if committed {
                    check_ack(&view, &id, wait.token, majority, tag);
                    self.stats.add(Acked, 1);
                    self.acked.push((id.clone(), wait.token));
                } else if lead.0 == wait.token.epoch && lead.1.as_ref() == Some(&id) {
                    kept.push(wait);
                }
            }
    ...
    /// **The late-ack fence**, off the same raw bytes the core just answered
    /// from: a voter stamped above the write's epoch never counts, so once the
    /// remainder cannot reach a majority the verdict must be `Pending`.
    fn check_fence(&mut self, writer: &NodeId, wait: &mut InFlight, seen: (&[LedgerView], bool)) {
        let (view, committed) = seen;
        let epoch = wait.token.epoch;
        let above = |m: &&LedgerView| m.reading.as_ref().is_some_and(|r| r.lead_epoch > epoch);
        let moved = view.iter().filter(above).count();
        if moved > 0 && !wait.fenced {
            wait.fenced = true;
            self.stats.add(FenceSeen, 1);
        }
        if view.len() - moved < self.majority {
            let (tag, now, voters) = (self.tag, self.now, view.len());
            assert!(
                !committed,
                "{tag:?}: {writer}'s write {:?} committed at {now} with {moved} of {voters} \
                 voters already stamped above its epoch — the fence let a round close after \
                 the group had moved on",
                wait.token,
            );
            self.stats.add(FenceBlocked, 1);
        }
    }
    ...
    fn start_write(&mut self, id: &NodeId, wait: bool) {
        ...
        *entry = (*entry).max(token);
        if wait && node.inflight.len() < MAX_INFLIGHT {
            let fenced = false;
            node.inflight.push(InFlight { token, fenced });
        }
    ...
    fn restart(&mut self, id: &NodeId) {
        ...
        let node = Node::new(id, self.lags[id], applied);
        self.nodes.insert(id.clone(), node);
        self.stats.add(Restart, 1);
    }

    /// One fault, drawn uniformly from eight arms and **half of them aimed at
    /// the sitting host**, which is the only way a hostship changes hands.
    ///
    /// Killing it is the clean migration; isolating it lapses its lease and lets
    /// the majority side elect a successor *while its own watch catches up*,
    /// which is what leaves a commit wait in flight at the old epoch. The
    /// one-way flavour is the asymmetric partition the tier names, and the shape
    /// that fills a deposed host's view with readings stamped above the epoch it
    /// is waiting at. An arm whose guard fails falls through to churn, which
    /// keeps anti-entropy busy.
    ///
    /// The wider menu — random victims, write bursts against a small ring,
    /// arbitrary link cuts, a stalled follower loop — belongs to `hosted_dst.rs`,
    /// which draws sixteen arms; this file deliberately spends its seeds on
    /// hand-overs instead. Adding the stall arm here was tried and reverted: it
    /// costs this corpus two thirds of its fence observations (`FenceBlocked`
    /// 11 → 2) without making the gate-bypass control bite, and the fence is the
    ...
    fn inject_fault(&mut self, rng: &mut SplitMix64) {
        ...
        match (rng.below(SLOTS), host) {
            ...
            (5, Some(host)) if live.len() > 1 => self.cut(&host, &live, true),
            (2 | 6, host) => {
...
/// One migration-heavy run: elect under a fair fabric, then [`ROUNDS`] faults
/// from [`Harness::inject_fault`]'s eight arms, with every property sampled at
/// every round — then heal, quiesce, and hold S5 once more.
pub(crate) fn migrate_scenario(seed: u64) -> Stats {
    let mut rng = rng(seed ^ 0x2c99);
    let n = if rng.below(2) == 0 { 3 } else { 5 };
    let mut h = Harness::new(("S5-migrate", seed), n, &mut rng, (RING, LAG));
