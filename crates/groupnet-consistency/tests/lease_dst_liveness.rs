...
//! assertions readable in one place instead of behind a shared module that has
//! to serve both.
//!
...
//! # The property
//!
//! * **L-L1 — liveness.** On a healthy run every reader reaches `Serving`, no
//!   lease ever lapses, and coherent writes resolve on the ack fast path; after
//!   a partition and a heal, leases re-establish and writes are fast again.
//!
//! The safety properties travel with it rather than being switched off: every
//! observation this suite takes still runs L-P1's lapse contract, L-P2's
//! window arithmetic and L-P3's lease-life rule, identically to the chaos file.
//! What makes this suite *liveness* is the schedule and the end-of-run
//! assertion — that the run reached service and resolved its writes at all,
//! which no safety property can fail to hold vacuously.
...
struct Harness {
    ...
    /// Nothing here crashes, but the schedule next door does, and the two
    /// harnesses stay the same shape so an assertion can be read against either.
...
/// **L-L1.** On a healthy, quiesced run every reader reaches `Serving` and
/// *stays* there — no lease lapses at all — and a coherent write resolves on
/// the ack fast path, never on the lapse path. A partition then lapses the
/// leases it cuts; after the heal, leases re-establish and writes are fast
/// again. 64 seeds, 3..=7 nodes.
#[test]
fn dst_lease_healthy_runs_serve_and_reestablish_after_a_heal() {
    let mut total = Stats::default();
    for seed in 0..64u64 {
        total.absorb(&healthy_scenario(seed));
    }
    assert_eq!(
        (total.all_applied, total.resolved_by_lapse, total.timed_out),
        (128, 0, 0),
        "every seed resolves exactly two writes, both on the ack fast path"
    );
}
...
fn healthy_scenario(seed: u64) -> Stats {
    ...
    let mut h = Harness::new(tag, n, &mut rng, 0);

    // 1. Quiesce: every reader takes a lease, and keeps it.
    h.run_rounds(40, 50);
    assert_all_serving(&h, "the initial quiesce");
    for node in h.nodes.values() {
        assert_eq!(
            node.lease.lapses(),
            0,
            "L-L1 seed {seed}: {} lapsed on a fault-free fabric",
            node.id
        );
    }

    // 2. A coherent write resolves — on the ack fast path, by the suite-level
    //    assertion that no run here ever reaches the lapse path at all.
    let writer = h.all[0].clone();
    h.start_write(&writer);
    h.run_rounds(20, 50);
    assert_resolved(&h, &writer, "the initial quiesce");

    // 3. Cut the cluster in two for 900ms — well past one lease duration, so
    //    every lease that straddles the cut must lapse.
    let (left, right) = h.all.split_at(h.all.len() / 2);
    for a in left {
        for b in right {
            h.sim.block(a, b);
            h.sim.block(b, a);
    ...
    h.run_rounds(18, 50);
    assert!(
        h.nodes.values().any(|node| node.lease.lapses() > 0),
        "L-L1 seed {seed}: a partition longer than a lease duration lapsed nobody"
    );

    // 4. Heal: leases re-establish, and writes are fast again.
    h.sim.heal_all();
    h.run_rounds(60, 50);
    assert_all_serving(&h, "the heal");
    h.start_write(&writer);
    h.run_rounds(20, 50);
    assert_resolved(&h, &writer, "the heal");
    h.stats
}

fn assert_all_serving(h: &Harness, after: &str) {
    for node in h.nodes.values() {
        assert_eq!(
            node.lease.peek(ClockMs(h.now)),
            LeaseState::Serving,
            "L-L1 seed {}: {} held no lease at {} after {after} (confirmed {:?}, \
             window {:?}, roster {:?})",
            h.tag.seed,
            node.id,
            h.now,
            node.lease.confirmed(),
            node.lease.serve_until(),
            node.roster,
        );
    }
}

fn assert_resolved(h: &Harness, writer: &NodeId, after: &str) {
    assert!(
        h.nodes[writer].inflight.is_none(),
        "L-L1 seed {}: the coherent write started after {after} never resolved",
        h.tag.seed
    );
}
